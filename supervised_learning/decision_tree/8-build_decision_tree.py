...
class Decision_Tree():
    ...
    def possible_thresholds(self, node, feature):
        """Returns possible thresholds for a given feature"""
        values = np.unique(
            (self.explanatory[:, feature])[node.sub_population])
        return (values[1:] + values[:-1]) / 2

    def Gini_split_criterion_one_feature(self, node, feature):
        """Returns best threshold and Gini score for one feature"""
        thresholds = self.possible_thresholds(node, feature)
        population = self.explanatory[:, feature][node.sub_population]
        target = self.target[node.sub_population]
        classes = np.unique(target)
        n = len(target)

        left_masks = np.array(
            [population > t for t in thresholds])
        right_masks = ~left_masks

        left_counts = np.array([
            [np.sum((left_masks[j]) & (target == c))
             for c in classes]
            for j in range(len(thresholds))
        ])
        right_counts = np.array([
            [np.sum((right_masks[j]) & (target == c))
             for c in classes]
            for j in range(len(thresholds))
        ])
        ...
        left_gini = 1 - np.sum(
            (left_counts / left_sizes_safe[:, None]) ** 2, axis=1)
        right_gini = 1 - np.sum(
            (right_counts / right_sizes_safe[:, None]) ** 2, axis=1)

        gini_avg = (
            left_sizes * left_gini + right_sizes * right_gini) / n

        best = np.argmin(gini_avg)
        return thresholds[best], gini_avg[best]

    def Gini_split_criterion(self, node):
        """Returns best feature and threshold using Gini impurity"""
        x = np.array([
